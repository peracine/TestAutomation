...
{
    public class TextControllerWebApplicationFactoryTest : IClassFixture<TestAutomationFactory<Startup>>
    {
        private readonly HttpClient _client;

        public TextControllerWebApplicationFactoryTest(TestAutomationFactory<Startup> testAutomationFactory)
        {
        ...
        {
            var allWords = string.Join(" ", DataMock.ListTexts()).Split(" ", StringSplitOptions.RemoveEmptyEntries).Distinct().ToArray();
            string text = allWords[new Random().Next(0, allWords.Length - 1)];

            var result = await _client.GetAsync($"Text?q={text}");
        ...
        {            
            ...
            var result = await _client.PostAsync("Text", httpContent);
            var content = await result.Content.ReadAsStringAsync();
